...
const MeetingExpenses = () => {
  ...
  return <div className="flex min-h-screen bg-stone-50"><Sidebar /><div className="min-w-0 flex-1"><DashboardHeader /><main className="px-4 py-6 sm:px-6 md:p-8"><h1 className="text-2xl font-bold text-gray-900">Meeting Expenses</h1><p className="mb-6 text-gray-500">A transparent monthly summary for your hub. Individual expenses and receipts are kept private.</p><div className="mb-6 max-w-xs"><label className="text-sm font-medium text-gray-700">Month<input type="month" value={month} onChange={(event) => setMonth(event.target.value)} className="vc-select mt-1.5 w-full rounded-xl px-4 py-2.5" /></label></div>{error ? <p className="rounded-xl border border-red-100 bg-red-50 px-4 py-3 text-sm text-red-600">{error}</p> : <div className="max-w-3xl"><MonthlySummary summary={summary} loading={loading} /></div>}</main></div></div>;
};
